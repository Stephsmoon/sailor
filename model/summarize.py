...
# write messages without breaking tqdm progress bars
def progressWrite(message):
	if tqdm != None:
		tqdm.write(str(message))
	else:
		print(message)

# default prompt for LLM summarizing
defaultPrompt = (
...
# check if text contains kanji/non-english characters
def containsCJK(text):
	return re.search(r'[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]', text) is not None

# delete reasoning tags generated by LLM
...
# load LLM model and tokenizer onto a gpu
def loadSummarizer(
...
# add instructions to not produce chain-of-thought reasoning generation
def prepareMessages(messages, enableThinking=False):
	# if thinking allowed, return messages unchanged
	# - - - - - - - - - - #
	# ideally we want to ignore all possible thinking
	# this is because thinking only adds to overhead
	# we want to summarize from existing content not from pretrained knowledge
	# - - - - - - - - - - #
	if enableThinking:
		return messages
	preparedMessages = []
...
# convert into model-ready text prompt
...
# runs model on chat prompt and returns polished output
...
# repair non-english hallucinations
# - - - - - - - - - - #
# this was made to tackle a weird issue of models
# replying in chinese. Previous tested models never
# did this too many times but from here it seems
# to be specific words that trigger this. Probably a better fix
# - - - - - - - - - - #
def repairToEnglish(
	summaryText,
	tokenizer,
...
# cleans summary text to be more readable
...
def summarizeChunk(
	...
	if not containsCJK(summary):
		...
		return summary
	progressWrite("[WARNING] Non-English Output detected; Correcting...")
	progressWrite(summary[:300])
...
def summarizeWithLoadedModel(
	...
	finalSummaries = {}
	chunkItems = list(chunks.items())

	if progressPosition == None:
		try:
			progressPosition = int(deviceId)
		except Exception:
			progressPosition = 0

	if showProgress and tqdm != None and len(chunkItems) > 0:
		chunkIterator = tqdm(
			chunkItems,
			desc=f"GPU {deviceId} - {progressLabel}",
			unit="chunk",
			position=progressPosition,
			leave=True
		)
	else:
		chunkIterator = chunkItems

	for chunkName, chunkText in chunkIterator:
		...
		if containsCJK(summary):
			...
			progressWrite(f"[WARNING] {warningMessage}")
